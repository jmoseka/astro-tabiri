  ...
  return (
    ...
    >
      <nav className=" w-[98%] md:w-[97%]  h-fit text-[.8rem] nav-menu flex justify-end gap-24  ">
        ...
        >
          <div
            className={`flex cursor-pointer flex-col  items-center w-[3.1rem] h-[3.1rem] `}
          >
            <div
              className={`translate-y-4 relative humberger ${isOpen ? 'open' : ''}`}
              id="menu-btn"
            >
        ...
        <div
          className={`nav-items translate-y-10 justify-center  gap-10 
        ...
        >
          <div>
            <div className={`relative mx-auto}`}>
              ...
              <button
                onClick={() => displayZodiac()}
                className="zod-link nav-menu-link py-1 px-6
                             w-full flex gap-1 justify-center items-center"
              >
                <p>Zodiac signs</p>
                {isZodiacListOpen ? (
                  <MdOutlineKeyboardArrowUp className="text-[1.4rem]" />
                ) : (
          ...
          <div className="py-1 hover:opacity-[0.8] dark:hover:text-lightActive">
            <Link
              onClick={() => dropMenu()}
              className="nav-menu-link uppercase"
              to="/"
            >
              Daily Horoscope
            </Link>
